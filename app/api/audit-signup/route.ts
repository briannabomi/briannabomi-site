...
function document(title: string, message: string, status: number) {
  return new Response(
    `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${title}</title><main><h1>${title}</h1><p>${message}</p><p><a href="/sexbydesign#audit-signup">Return to the signup form</a></p></main></html>`,
    {
      status,
      headers: {
        ...noStore,
